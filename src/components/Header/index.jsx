...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-[100] h-20 flex items-center bg-[var(--bg-glass)] backdrop-blur-xl border-t-2 border-t-[var(--accent-gold)] border-b border-b-[var(--border-gold)] shadow-[0_10px_30px_rgba(0,0,0,0.6)]">
      <div className="max-w-[1200px] mx-auto px-6 w-full flex items-center justify-between">
        ...
        <div className="flex items-center">
          ...
          >
            <img
              ...
              alt="XHERO"
              className="size-10 md:size-[60px] object-contain shrink-0"
            />
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="hidden md:block text-xs leading-3 font-light tracking-wider capitalize text-[var(--text-secondary)] -mt-1">
                Livestream Platform
              </span>
            </div>
          </button>
        ...
        {/* Desktop Nav: hidden by default, flex on md+ */}
        {/* <nav className="hidden md:flex items-center gap-6 desktop-nav"> */}
        {/* Your nav links here */}
        {/* </nav> */}

        {/* Mobile Menu Button: block by default, hidden on md+ */}
        <button className="block md:hidden p-2 -mr-2 mobile-menu-btn">
          <svg className="size-6 fill-[var(--accent-gold)]">
            {/* Menu Icon Path */}
          </svg>
        </button>
        {/* </div> */}
        ...
        {/* <div className="nav-links desktop-nav">
          <Link href="/" className={`nav-link ${pathname === '/' ? 'active' : ''}`}>Khám Phá</Link>
          <Link href="/publish" className={`nav-link ${pathname === '/publish' ? 'active' : ''}`}>Phát Sóng</Link>
          <Link href="/events" className={`nav-link ${pathname === '/events' ? 'active' : ''}`}>Lịch Học</Link>
          <Link href="/profile" className={`nav-link ${pathname === '/profile' ? 'active' : ''}`}>Chuyên Gia</Link>
        </div> */}
        ...
        <div className="flex items-center gap-5">
          ...
          {isMobile && <button className="mobile-menu-btn text-[var(--text-primary)]" onClick={toggleDrawer}>
          ...
          <div className="mobile-drawer fade-in">
            <div className="drawer-content">
              <div className="drawer-header">
                <span className="text-xl font-bold text-[var(--accent-gold)]">DANH MỤC</span>
              </div>
              {/* <div className="drawer-links">
              <Link href="/" className={`drawer-link ${pathname === '/' ? 'active' : ''}`} onClick={closeDrawer}>Khám Phá</Link>
              <Link href="/publish" className={`drawer-link ${pathname === '/publish' ? 'active' : ''}`} onClick={closeDrawer}>Phát Sóng</Link>
              <Link href="/events" className={`drawer-link ${pathname === '/events' ? 'active' : ''}`} onClick={closeDrawer}>Lịch Học</Link>
              <Link href="/profile" className={`drawer-link ${pathname === '/profile' ? 'active' : ''}`} onClick={closeDrawer}>Chuyên Gia</Link>
            </div> */}
            </div>
          </div>
        )
