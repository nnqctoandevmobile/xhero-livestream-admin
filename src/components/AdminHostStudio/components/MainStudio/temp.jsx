...
export default function MainStudio() {
  ...
  useEffect(() => {
    if (!detailData?.streamSettings?.streamId) return;
    const chatRef = ref(rtdb, `rooms/${detailData?.streamSettings?.streamId}/chat`);
  ...
  }, [detailData?.streamSettings?.streamId]);
  ...
  useEffect(() => {
    if (!detailData?.streamSettings?.streamId) return;
        console.log(detailData?.streamSettings?.streamId);

    // Sync with Firebase for room info
    const roomRef = ref(rtdb, `rooms/${detailData?.streamSettings?.streamId}`);
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (data.state?.isLive === true || data.state?.status === SESSION_STATUS.Live.value) {
          setIsWaiting(false);
        } else {
          setIsWaiting(true);
        }
  ...
  const handleCancelSession = () => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const roomStateRef = ref(rtdb, `rooms/${detailData?.streamSettings?.streamId}/state`);
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col h-screen bg-[#090D14] text-white overflow-hidden font-sans">
        ...
        {isWaiting && (
          <CountdownView roomId={detailData?.streamSettings?.streamId} />
        )}
        ...
        <div className="w-full aspect-video bg-black relative border-b border-[#1E2633] shrink-0">
          ...
          <div className="absolute top-3 left-3 flex gap-2 z-30">
            {roomInfo?.state?.status === SESSION_STATUS.Live.value && (
            ...
            <div className="p-4 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  disabled={roomInfo?.state?.status === SESSION_STATUS.Cancelled.value}
  ...
  return (
    <div className="grid grid-cols-[1fr_3fr_1fr] h-screen bg-[#090D14] text-white overflow-hidden font-sans">
      {/* Pre-live Countdown Overlay */}
      {isWaiting && (
        <CountdownView roomInfo={roomInfo} roomId={detailData?.streamSettings?.streamId} />
      )}
